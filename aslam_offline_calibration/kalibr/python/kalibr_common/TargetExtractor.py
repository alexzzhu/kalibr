...
def extractCornersFromDatasetWithIndices(dataset, detector, multithreading=False, numProcesses=None, clearImages=True, noTransformation=False):
    print "Extracting calibration target corners"    
    targetObservations = []
    numImages = dataset.numImages()
    ...
    if multithreading:   
        ...
        if resultq.qsize() > 1:
            targetObservations = [[]]*(resultq.qsize()-1)
            indices = [[]]*(resultq.qsize()-1)
            for lidx, data in enumerate(iter(resultq.get, 'STOP')):
                obs=data[0]; time_idx = data[1]
                targetObservations[lidx] = (time_idx, obs)
                indices[lidx] = time_idx
            targetObservations = list(zip(*sorted(targetObservations, key=lambda tup: tup[0]))[1])
    ...
    return (indices, targetObservations)
